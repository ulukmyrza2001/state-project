...
import DetailHeaderOfCommittee from './DetailHeaderOfCommittee'
...
const InnerHeaderOfCommittee = () => {
   ...
   useEffect(() => {
      dispatch(getOneData({ category: 'youthAffairsHC', id }))
   }, [])
   ...
   const pathsArray = [
      ...
      },
      {
         ...
         name: `${oneData?.managerName}`,
      },
   ]
   return (
      <TempleteInnerPage pathsArray={pathsArray}>
         {isLoading && <LoadingInnerPage />}
         {!isLoading && <DetailHeaderOfCommittee oneData={oneData} />}
      </TempleteInnerPage>
   )
}
